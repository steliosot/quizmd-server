...
from .models import ClientEvent, CreateRoomRequest, CreateRoomResponse, JoinRoomRequest, JoinRoomResponse, RoomSnapshot, ServerEvent
...
async def create_room(request: Request, payload: CreateRoomRequest) -> CreateRoomResponse:
    ...
    return CreateRoomResponse(
        room_code=room_code,
        mode=created["mode"],
        join_url=f"{base}/join/{room_code}?token={room_token}",
...
async def join_link_info(room_code: str, token: str) -> JSONResponse:
    """Simple endpoint so shared join URLs resolve with human-readable instructions."""
    try:
        snapshot = manager.get_snapshot(room_code)
    ...
    return JSONResponse(
        content={
            ...
            "message": (
                "Use quizmd CLI to join: quizmd room --join <room_code_or_url>. "
                "Include this token when prompted."
            ),
            "room_token": token,
        }
    )
